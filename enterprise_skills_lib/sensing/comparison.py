        ...
        if match_key:
            ...
            if item_a.get("ring") != item_b.get("ring"):
                radar_diff.append(RadarDiffItem(
                    name=item_b["name"], status="moved", quadrant=item_b.get("quadrant", ""),
                    current_ring=item_b.get("ring"), previous_ring=item_a.get("ring"),
                    description=item_b.get("description", ""),
                ))
            else:
                radar_diff.append(RadarDiffItem(
                    name=item_b["name"], status="unchanged", quadrant=item_b.get("quadrant", ""),
                    current_ring=item_b.get("ring"), previous_ring=item_a.get("ring"),
                    description=item_b.get("description", ""),
                ))
        else:
            radar_diff.append(RadarDiffItem(
                name=item_a["name"], status="removed", quadrant=item_a.get("quadrant", ""),
                previous_ring=item_a.get("ring"), description=item_a.get("description", ""),
            ))

    for name_b, item_b in radar_b.items():
        if name_b not in matched_b:
            radar_diff.append(RadarDiffItem(
                name=item_b["name"], status="added", quadrant=item_b.get("quadrant", ""),
                current_ring=item_b.get("ring"), description=item_b.get("description", ""),
            ))
