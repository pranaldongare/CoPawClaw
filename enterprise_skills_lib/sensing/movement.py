...
async def detect_radar_movements(
    ...
    for radar_item in new_report.radar_items:
        ...
        if prev_ring and prev_ring != radar_item.ring:
            ...
            moved_count += 1
            logger.info(f"Movement detected: '{radar_item.name}' {prev_ring} -> {radar_item.ring}")

    logger.info(f"Movement detection complete: {moved_count} items moved")
    return new_report


def _find_previous_ring(name: str, prev_map: dict[str, str]) -> Optional[str]:
...
async def _load_previous_report(user_id: str, domain: str) -> Optional[dict]:
