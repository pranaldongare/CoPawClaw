...
async def create_shared_report(report_tracking_id: str, owner_user_id: str) -> SharedReport:
    """Create a new shared report."""
    os.makedirs(SHARED_DIR, exist_ok=True)
    share_id = str(uuid.uuid4())[:8]
    shared = SharedReport(share_id=share_id, report_tracking_id=report_tracking_id, owner_user_id=owner_user_id)
...
async def add_vote(share_id: str, user_id: str, user_name: str, radar_item_name: str, suggested_ring: str, reasoning: str = "") -> Optional[RadarVote]:
    shared = await load_shared_report(share_id)
    if not shared:
        return None
    vote = RadarVote(user_id=user_id, user_name=user_name, radar_item_name=radar_item_name, suggested_ring=suggested_ring, reasoning=reasoning)
    shared.votes.append(vote)
    await _save_shared(shared)
    return vote


async def add_comment(share_id: str, user_id: str, user_name: str, text: str, radar_item_name: str = "") -> Optional[RadarComment]:
    shared = await load_shared_report(share_id)
    if not shared:
        return None
    comment = RadarComment(user_id=user_id, user_name=user_name, radar_item_name=radar_item_name, text=text)
...
async def get_feedback(share_id: str) -> Optional[dict]:
    ...
    for vote in shared.votes:
        ...
        vote_summary[name]["ring_counts"][ring] = vote_summary[name]["ring_counts"].get(ring, 0) + 1
